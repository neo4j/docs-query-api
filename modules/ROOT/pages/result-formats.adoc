...
[NOTE]
To represent `Nan`, `Infinity` and `-Infinity`, the `_value` field contains the string values "NaN", "Infinity", and "-Infinity". "-NaN" is handled as `NaN` when used as parameter, but this value isn't produced the API.
