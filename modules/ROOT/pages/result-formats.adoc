...
Or use the JSON with type information.
|===
...
[#typed-json-version]
=== Media-type versions
label:new[Introduced in 2025.10]

To ensure safe type evolution, the MIME type is versioned.  
This allows new types to be added in a predictable and backward-compatible way.
...
Nothing new added, alias for `application/vnd.neo4j.query`
...
----
label:enterprise-edition[] label:new[Introduced in 2025.11] label:new[Introduced in "application/vnd.neo4j.query.v1.1"]

[NOTE]
The `coordinatesType` field accepts the following values: `FLOAT64`, `FLOAT32`, `INT64`, `INT32`, `INT16` and `INT8`.
...
----
label:new[Introduced in 2025.11] label:new[Introduced in "application/vnd.neo4j.query.v1.1"]
