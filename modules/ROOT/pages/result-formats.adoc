...
[NOTE]
link:https://neo4j.com/docs/cypher-manual/25/values-and-types/vector/[`VECTOR`] values can only be retrieved from the database, but cannot be passed as parameters in plain json.
To create vectors, use the link:https://neo4j.com/docs/cypher-manual/25/values-and-types/vector/#construct-vector-values[Cypher `vector` constructor], or use the xref:typed-json[JSON with type information] format.
|===
...
To receive the result in this format, set `Accept: application/vnd.neo4j.query.v1.1` in the request headers.
...
[NOTE]
The values can be represented on the *decimal notation* or *scientific notation*.
`Nan`, `Infinity` and `-Infinity` are represented "NaN", "Infinity", and "-Infinity".
"-NaN" is handled as `NaN` when used as parameter, but this value isn't produced the API.
[NOTE]
When returning values, values with magnitude between `10⁻³` (inclusive) and `10⁷` (exclusive) will be represented using the *decimal notation*. 
Other values will be represented using the *scientific notation*.
...
Initial implementation of the typed JSON format.
