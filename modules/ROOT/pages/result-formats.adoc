...
----
[NOTE]
Introduced at `Neo4j 2025.10`.
[WARNING]
The `VECTOR` type uses the same encoding format as `LIST`, but it is only supported for **read** operations.  
To **write** a `VECTOR` value to the database in JSON format, use the `CYPHER` vector constructor.
For more information, see the link:https://neo4j.com/docs/cypher-manual/current/introduction/[Cypher documentation].
|===
