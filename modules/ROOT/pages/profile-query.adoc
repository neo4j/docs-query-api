...
Profiling a query and analyzing its execution plan can help locating potential bottlenecks or performance improvements.
For more information, see link:{neo4j-docs-base-uri}/cypher-manual/current/planning-and-tuning/[Cypher -> Execution plans and query tuning].


== The `EXPLAIN` keyword

If a query is prepended with `EXPLAIN`, the server returns the execution plan it would use to run it, but not actually execute it.
The plan is available as a nested object of link:https://neo4j.com/docs/cypher-manual/current/planning-and-tuning/operators/[Cypher operators] under the key `queryPlan`.
...
If a query is prepended with `PROFILE`, the server returns the execution plan _and_ the corresponding query result.
The plan is available as a nested object of link:https://neo4j.com/docs/cypher-manual/current/planning-and-tuning/operators/[Cypher operators] under the key `profiledQueryPlan`.
