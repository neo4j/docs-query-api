...
[NOTE]
The Query API is enabled by default on Aura, but disabled on on-prem instances.
To enable it on on-prem, add `QUERY_API_ENDPOINTS` to the configuration setting link:https://neo4j.com/docs/operations-manual/current/configuration/configuration-settings/#config_server.http_enabled_modules[`server.http_enabled_modules`].
