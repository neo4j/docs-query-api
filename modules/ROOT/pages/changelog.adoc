...
- Introduce `application/vnd.neo4j.query.v1.2``
    * Introduce `UUID` type.
- Introduce `queryType`
- Introduce `resultAvailableAfter` and `resultConsumedAfter`
- Fix Vector de-serialization. Remove the need for submitting the fields in order. 
- Fix `profileQueryPlan` optional fields. Non-present fields being omitted instead of defaulting to `0`.
...
- Add handling for `Unsupported Type` on typed and untyped json.
...
- Add support for transaction type (`txType`).
...
- Query API is introduced as preview on the database. 
