...
It must be included as a header to all subsequent requests for this transaction, including commit/rollback requests.
<.> Query result keys.
<.> Query result values for each result returned by the query. Each individual result values are in the same order as `fields`. +
For more information on what format the values may take, see xref:plain-json.adoc[] and xref:typed-json.adoc[].
...
You can provide optional configuration parameters when creating a transaction.
You can specify:

include::{partials}/transactions.adoc[tag=transactions-conf]
