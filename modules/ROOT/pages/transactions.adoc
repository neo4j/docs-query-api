...
<.> The header `neo4j-cluster-affinity` identifies the cluster member which is handling the transaction.
It must be included as a header to all subsequent requests, up until when the transaction is committed or rolled back.
<.> Query result keys.
...
The ID is used as part of subsequent request URIs.
====
...
By default the timeout is 30 seconds, but you can set a different value in the server configuration (link:{neo4j-docs-base-uri}/operations-manual/current/configuration/configuration-settings#config_server.http.transaction_idle_timeout[`server.http.transaction_idle_timeout`]).
